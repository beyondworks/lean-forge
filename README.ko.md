...
[Ponytail](https://github.com/DietrichGebert/ponytail)을 통째로 안에 담았고,
[dryforge](https://github.com/prekuter/dryforge)의 질문 방식을 가져왔습니다. 따로 설치할 것도, 어느 쪽을 켤지 고를 것도 없습니다.
...
- **꼼꼼한 하네스는 절차에 비용을 씁니다.** 필요한 질문을 정확히 합니다. 그런 다음 계약 문서를 쓰고, 검수
  서브에이전트를 띄우고, 가지를 만들어 커밋하는 데 30분을 씁니다.

둘 다 재 봤습니다. 꼼꼼한 하네스는 결과를 가르는 질문을 **처음 1.5~3분 안에** 했고, 시간의 60~75%는 그 뒤의
절차에 썼습니다. 빠른 하네스가 빨랐던 이유는 묻지 않았기 때문입니다.
...
flowchart TB
    ...
    D["dryforge<br/>(발상만, 코드 없음)"] -. "결정 관점,<br/>「다른 규칙이 있나요?」" .-> F
```
...
| dryforge | 질문 방식: 관점별로 결정 나열하기, 입력을 사실이 아닌 재료로 보기, 예시를 붙여 추천하기, 빠진 규칙 묻기 | 계약 문서, 검수 서브에이전트, 가지·커밋 절차는 넣지 않았습니다 |
...
|---|---|---|
| dryforge | 같음 (93% vs 97%, p = 0.66) | **15배 빠르고 19배 쌈** (p = 0.008) |
| Castra + Ponytail | **2.2배 높음** (93% vs 42%, p < 0.001) | 유의미한 차이 없음 (p = 0.07) |

다른 플러그인·훅·메모리가 모두 켜진 작성자의 개인 설정에서도 같은 결과가 나왔습니다. 정답률은 dryforge 97%
대비 95%(p = 1.00)였고 시간은 272분 대비 18분(p = 0.016)이었습니다. Castra + Ponytail 대비로는 52% 대비
95%(p < 0.001)였고 시간은 같았습니다.
...
- 과제 7개에서 추가된 실제 코드: lean-forge 139줄, dryforge 349줄.
...
- [dryforge](https://github.com/prekuter/dryforge), prekuter — MIT, 발상만 참고
